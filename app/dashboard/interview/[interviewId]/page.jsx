...
function Interview() {
  ...
  const GetInterviewDetails = async () => {
    try {
      ...
      console.log(result);
      setInterviewData(result[0]); // Set fetched data
    } catch (error) {
      ...
      <div className="flex flex-col my-5 gap-5">
        ...
        {interviewData ? (
          <>
            ...
            <h2 className="text-lg">
              <strong>Job Desc:</strong> {interviewData.jobDesc}
            </h2>
            <h2 className="text-lg">
              <strong>Job Exp:</strong> {interviewData.jobExperience}
            </h2>
          </>
        ) : (
          <p>Loading interview details...</p> // Show a loading message or spinner
        )}
        </div>
